...
val_imgs, _, _ = get_data("C:/Users/fjbar/Downloads/keras-frcnn-master_clean/keras-frcnn-master/aug_testing_images_new_rev1.csv")
	...
	while True:
		try:
			...
			if len(rpn_accuracy_rpn_monitor) == epoch_length and C.verbose:
				...
				rpn_accuracy_rpn_monitor = []
				print('Average number of overlapping bounding boxes from RPN = {',mean_overlapping_bboxes,'} for {',epoch_length,'} previous iterations')
			...
			if iter_num == epoch_length:
				...
				if C.verbose:
					...
					print('Loss RPN regression: {',loss_rpn_regr,'}')
					print('Loss Detector classifier: {',loss_class_cls,'}')
					print('Loss Detector regression: {',loss_class_regr,'}')
				...
				if curr_loss < best_loss:
					if C.verbose:
						print('Total loss decreased from {',best_loss,'} to {',curr_loss,'}, saving weights')
					best_loss = curr_loss
				model_all.save_weights('D:/Documents/WPI/Artifical_Intelligence_in_Autonomous_Vehicles/python_elephants_rev0/'+model_path_regex.group(1)[2:] + "_" + '{:04d}'.format(epoch_num) + model_path_regex.group(2))
